...
func MoveDirectory(moveLimiter chan int, srcDir string, destParentDir string) error {
	...
	if !srcDirInfo.IsDir() {
		return fmt.Errorf("source directory '%s' is not a directory", srcDir)
	}
	...
	if !destParentDirInfo.IsDir() {
		return fmt.Errorf("destination parent directory '%s' is not a directory", destParentDir)
	}
	...
	if err == nil {
		if !destDirInfo.IsDir() {
			return fmt.Errorf("destination parent directory '%s' exists and is not a directory", destDir)
		}
	} else {
	...
	if len(moveErrors) > 0 {
		return fmt.Errorf("errors moving children of directory '%s'", srcDir)
	}
...
func MoveFile(srcFile string, destDir string) error {
	...
	if !isSrcSymLink && !srcFileMode.IsRegular() {
		return fmt.Errorf("source file '%s' is not a regular file or symlink", srcFile)
	}
	...
	if err == nil {
		...
		if isSrcSymLink && destFileMode&os.ModeSymlink != 0 {
			...
			return fmt.Errorf("destination file '%s' exists and is not a regular file", destFile)
		}
	}
